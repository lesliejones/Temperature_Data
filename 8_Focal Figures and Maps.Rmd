...
CO_p <- preds %>% mutate(month=month(date)) %>% mutate(decade=case_when(year< 1990 ~ 1980, year >= 1990 & year < 2000 ~ 1990, year >=2000 & year < 2010 ~ 2000, year>2009 ~ 2010)) %>% filter(CO_p == 1)
...
ggplot(data=jul_2) + geom_bar(aes(x=bin, y=percent, fill=as.factor(decade)), stat = "identity")+
  facet_wrap(~decade) + 
