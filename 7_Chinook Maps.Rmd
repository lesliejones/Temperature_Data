...
rcas <- sf::st_read(dsn = fgdb, layer = "anchor_rcas_attributed_06022020") %>% select(c(rca_id, Shape, Shape_Area))
...
warm_cold <- K_p_rcas %>% filter(year==2004 | year==1992) %>% group_by(rca_id, year) %>% 
  summarize(yr_mn=mean(preds), yr_sd=sd(preds))

#average temperatures by decade and mapped
decade<- K_rcas %>%  group_by(rca_id, decade) %>% summarize(dec_mn=mean(preds), dec_sd=sd(preds))

#average temperatures by decade and month
mth_decade<- K_rcas %>%  group_by(rca_id, month, decade) %>% 
  summarize(dec_mn=mean(preds), dec_sd=sd(preds))

#animated plot1
K_aug <- rca_preds %>% filter(month==8) %>% group_by (rca_id, year, month) %>% summarise(aug_mn = mean(preds))
