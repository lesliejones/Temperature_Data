...
temp.dm <- temp.rca %>% 
  left_join(daymet %>% select(-date), by = c("rca_id" = "rca_id", "sampleDate" = "date_day3")) 

# rm(daymet)
```


## NOT YET DONE: Add in 8-day moving average of daymet temperatures because they may have a better correlation to stream temps
...
temp.dm <- temp.dm %>% 
  left_join(lst %>% select(-date), by = c("rca_id" = "rca_id", "sampleDate" = "date_day8")) %>% 
