...
```{r}
temp.rca <- temp.rca %>% 
  left_join(rca_reach %>% select(reach_id, reach_slope, reach_length, Forest, Shrub, Wetland, Riparian), 
...
```{r}
fgdb <- "Spatial_data/Anchor/anchor_DAYMET.gdb"
# List all feature classes in a file geodatabase
subset(ogrDrivers(), grepl("GDB", name))
fc_list <- ogrListLayers(fgdb)
print(fc_list)
...
Read in 5-day summed precipitation. First day of 5-day window, need to add four days before merging.
...
temp.dm <- temp.rca %>% 
  left_join(dm3day %>% select(-date), 
            by = c("rca_id" = "rca_id", "sampleDate" = "date_day3")) 
# rm(dm3day)
```
...
Bring in 5 day precipitation. Leslie says this is the first day of the 5 day window so need to add four days to match with daily mean temperatures.

```{r}
prcp5day <- prcp5day %>% 
  mutate(date_day5 = date + 4)
temp.dm <- temp.dm %>% 
  left_join(prcp5day %>% select(-date), 
            by = c("rca_id" = "rca_id", "sampleDate" = "date_day5")) 
...
```{r}
fgdb <- "Spatial_data/Anchor/anchor_LST.gdb"
# List all feature classes in a file geodatabase
subset(ogrDrivers(), grepl("GDB", name))
fc_list <- ogrListLayers(fgdb)
print(fc_list)
...
Check that everything is numeric and fix those that are not.
