...
Dustin updated all of the spatial covariates in the geodb: Anchor_LM_Outputs. Read them in from there.
...
Get attributes from Dustin's geodatabase: Anchor_LM_Outputs. Same location as other geodb. This db includes the fish species distributions by life stage and the new elevation covariate - mean elevation for the entire contributing area to each rca pour point.
...
# Read the feature class with new attributes
rca_fish <- sf::st_read(dsn = fgdb, layer = "anchor_rcas_31012020")

#Join new attributes to the temp.rca data frame
temp.rca <- temp.rca %>% 
  left_join(rca_fish %>% select(rca_id, K_r:S_p, Mean_Elev_Contributing_Area)) %>% 
  select(-Shape) %>% 
  rename(elev_mean_ca = Mean_Elev_Contributing_Area)

```

Create a new elevation covariate that matches what was used by Siegel and Volk - the difference between the mean elevation for the contributing area and the mean elevation for the rca (they used the site elevation, but it should be very comparable). Call this elev_delta and explore if some of the elevation and slope covariates are correlated.

```{r}

temp.rca <- temp.rca %>% 
  mutate(elev_delta = elev_mean_ca - elev_mean)

temp.rca %>% 
  distinct(Z_Min, Z_Max, Z_Mean, SLength, Min_Slope, Max_Slope, Avg_Slope, length_m, slope_P, slope_D,
           elev_mean, elev_max, elev_min, flowacc_max, elev_mean_ca, elev_delta) %>% 
  cor(.) %>% 
  corrplot.mixed(.)
