...
Just keep one mean elevation, slope in percent, and contributing area. Convert square meters to square kilometers for contributing area. 1 square kilometer = 1,000,000 square meters.
...
Note that contributing area is not being calculated correctly for the RCAs right now (1/6/20). This is a problem we had Dustin looking into for the IDW calculations, where we want watershed area draining to each RCA outlet as well. Problem is that the RCA polygons are grabbing higher flow accumulation values downstream, possibly we could get around this by grabbing the 95th percentile FAC value for each RCA to drop the highest outliers.

```{r}
temp.rca <- temp.rca %>% 
  select(-meanT, -meanNotNA, -ma_sum, -(Z_Min:length_m), -slope_D, -elev_min,
         -elev_max) %>% 
  mutate(cont_area = cont_area / 1000000)
```
