...
Check VIF on M1 before doing comparison of random effects. Already dropped forest and reach slope due to high correlations with other variables.

```{r}

M1 <- gls(mean ~ tair3c + I(tair3c^2) + jdc + I(jdc^2) + sweA1c + prcp5c + 
            wetlandc + shrubc + catchment_elev_mnc + cont_areac + ca_elev_mnc + season +
...
```
All spatial predictors now have VIF < 5. Some problems with tair/jd/season, but that was the case for the deshka as well.
...
#below not working, which is suspect that model is too complex or specified incorrectly.
plot_model(M3, type = "pred", terms = c("tair3c [all]", "season"))
plot_model(M3, type = "eff", terms = c("jdc [all]"))
...
plot_model(M3, type = "eff", terms = c("wetlandc [all]", "sweA1c"))
...
M3.2 <- update(M3.ml, .~. -prcp5c)
anova(M3.ml, M3.2)

M3.3 <- update(M3.2, .~. -season:I(tair3c^2))
anova(M3.2, M3.3)

M3.4 <- update(M3.3, .~. -season:tair3c)
anova(M3.3, M3.4)

tab_model(M3, digits = 4)
tab_model(M3.3, digits = 4)

plot_model(M3.3, type = "pred", terms = c("tair3c", "season"))
plot_model(M3.3, type = "pred", terms = "shrubc")

#saved model and tried plot_model in another project. Not sure why it won't plot single terms in this project, when it works fine in deshka project. Didn't help, same error in deshka project.
anchor.M3 <- M3.3
...
summer_xval_dredge %>% 
  group_by(model_num, fixed_eff_ct) %>% 
  summarize(mean_mae = mean(mae), mean_rmse = mean(rmse)) %>% 
  filter(fixed_eff_ct < 10, mean_rmse < 1.65) %>% 
  arrange(desc(fixed_eff_ct))

M3_models[[466]]
summary(eval(M3_models[[54]]))
summary(eval(M3_models[[62]]))
...
plot_model(eval(M3_models[[54]]), type = "pred", terms = tair3c)
```

save cross-validation results.

```{r}
save(summer_xval_dredge, file = "output/summer_xval_dredge.RData")
```
