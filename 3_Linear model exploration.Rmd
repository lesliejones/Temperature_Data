...
# Summer model - START HERE 7/7/20
...
temp.dm <- temp %>% 
  ...
  mutate(tair3sc = sjmisc::std(tair3),
         jdsc = sjmisc::std(jd),
         sweA1sc = sjmisc::std(sweA1),
         prcp5sc = sjmisc::std(prcp5),
         reach_slopesc = sjmisc::std(reach_slope),
         wetlandsc = sjmisc::std(Wetland),
         catchment_elev_mnsc = sjmisc::std(catchment_elev_mn),
         cont_areasc = sjmisc::std(cont_area)) 

```

Quick check on VIF for full model that will be used below.

```{r}
lm1 <- lm(mean ~ tair3sc + I(tair3sc^2) + jdsc + I(jdsc^2) + sweA1sc + prcp5sc + reach_slopesc +
            wetlandsc + catchment_elev_mnsc + cont_areasc + season,
          data = temp.dm, na.action = "na.fail")

vif(lm1)
```
Try a pairplot to see what is driving high vif for jd. Squared term for jd is correlated to raw air temps. And, probably need to drop either catchment elevation or stream_slope.

```{r}
temp.dm %>% 
  mutate(tair3sc.2 = tair3sc^2,
         jdsc.2 = jdsc^2) %>% 
  dplyr::select(tair3sc, tair3sc.2, jdsc, jdsc.2, sweA1sc, prcp5sc, reach_slopesc, wetlandsc,
         catchment_elev_mnsc, cont_areasc) %>% 
  pairs(upper.panel = panel.cor, lower.panel = panel.smooth)
```

Plot of air versus stream temperature by season.
...
Full model with all interactions. Dropped interactions with jd because models were too complex. Select best random term before using cross-validation on all model subsets for prediction.


```{r}

M1 <- lm(mean ~ tair3sc + I(tair3sc^2) + jdsc + I(jdsc^2) + sweA1sc + prcp5sc + reach_slopesc +
            wetlandsc + catchment_elev_mnsc + cont_areasc + season +
...
M3 <- lme(mean ~ tair3sc + I(tair3sc^2) + jdsc + I(jdsc^2) + sweA1sc + prcp5sc + reach_slopesc +
            wetlandsc + catchment_elev_mnsc + cont_areasc + season +
            sweA1sc:wetlandsc + season:(tair3sc + I(tair3sc^2)), 
...
Test for plot_model, which I can't get to run on complex models above.

```{r}
test <- lm(mean ~ tair3sc*season + I(tair3sc^2)*season,
          data = temp.dm, na.action = "na.fail")
summary(test)

test <- lm(mean ~ scale(tair3),
          data = temp.dm, na.action = "na.fail")
