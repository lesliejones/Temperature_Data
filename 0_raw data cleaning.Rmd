...
#adding in some extra data modifiers for figures.
CIKdat <- CIKdat %>% 
...
For KBERR, import the raw data. All of the raw data file names have been entered into a metadata worksheet where the file names match a KBERR site name, which matches a KBERR site name in Steve's shapefiles. For the raw data files, names them by their file name for now.
...
Missing data for SANC-1203-L and NINI-545-U because only loggers for those sites are in the top file. Add them in here because they are excel files, then fix dates and times.

```{r}

hws29 <- read_excel("Data/RAW Data/Anchor/KBERR/2008/Top/HWS029_20080806.xls", 
                    skip = 3, col_names = c("sampleDate", "sampleTime", 
                                            "Temperature"),
                    col_types = c("date", "date", "numeric"))

hws29 <- hws29 %>% 
  mutate(Site = "2008/Top/HWS029_20080806.xls")

dat08df <- bind_rows(dat08df, hws29)


hws28may <- read_excel("Data/RAW Data/Anchor/KBERR/2008/Top/HWS028_20080522.xls", 
                    skip = 3, col_names = c("sampleDate", "sampleTime", 
                                            "Temperature"),
                    col_types = c("date", "date", "numeric")) %>%
  mutate(Site = "2008/Top/HWS028_20080522.xls")

dat08df <- bind_rows(dat08df, hws28may)

hws28aug <- read_excel("Data/RAW Data/Anchor/KBERR/2008/Top/HWS028_20080807.xls", 
                    skip = 3, col_names = c("sampleDate", "sampleTime", 
                                            "Temperature"),
                    col_types = c("date", "date", "numeric")) %>% 
  mutate(Site = "2008/Top/HWS028_20080807.xls")

dat08df <- bind_rows(dat08df, hws28aug)
...
Add in extra date fields for figures.
...
Currently don't have lat/longs for one site that was imported because it had a logger in the bottom folder. There is another site (STAR-171-Middle - old) that has data in the top folder, but we don't have a lat/long for that site.
...
```{r}
topFiles <- c("Data/RAW Data/Anchor/KBERR/2008/Top/HWS030_20080519.xls", 
...
names(topList) <- sub(".*KBERR/", "", topFiles)
